...
class TestConfigLoading:
    ...
    def test_load_config_defaults_only(self):
        ...
        assert (
            config["retry"]["max_attempts"] == DEFAULT_CONFIG["retry"]["max_attempts"]
        )

    def test_load_config_with_file(self):
        """Test loading config from JSON file."""
        test_config = {
            "data": {"spotify_path": "custom/spotify.csv"},
...
class TestEnvironmentOverrides:
    ...
    def test_env_override_retry_config(self):
        ...
        with patch.dict(
            os.environ, {"MUSICREC_MAX_RETRIES": "5", "MUSICREC_BACKOFF_SECONDS": "2.5"}
        ):
            config = load_config()
    ...
    def test_env_override_app_config(self):
        ...
        with patch.dict(
            os.environ,
            {"MUSICREC_DEFAULT_PORT": "9000", "MUSICREC_DEFAULT_LIMIT": "25"},
        ):
            config = load_config()
    ...
    def test_env_override_base_path(self):
        ...
        with patch.dict(os.environ, {"MUSICREC_DATA_PATH": "/custom/data"}):
            ...
            assert (
                config["data"]["mood_path"] == "/custom/data/autotagging_moodtheme.tsv"
            )
...
class TestEnvironmentMappings:
    ...
    def test_env_mapping_coverage(self):
        ...
        for config_path in important_configs:
            assert (
                config_path in mapped_configs
            ), f"Missing environment mapping for {config_path}"
