...
class TestCreateSampleDataValidation:
    ...
    def test_tracks_per_genre_too_small(self):
        ...
        with pytest.raises(ValueError, match="tracks_per_genre must be at least 1, got 0"):
            create_sample_data(num_genres=2, tracks_per_genre=0)
            
        with pytest.raises(ValueError, match="tracks_per_genre must be at least 1, got -1"):
            create_sample_data(num_genres=2, tracks_per_genre=-1)
            
        with pytest.raises(ValueError, match="tracks_per_genre must be at least 1, got -5"):
    ...
    def test_tracks_per_genre_too_large(self):
        ...
        with pytest.raises(ValueError, match="tracks_per_genre cannot exceed 1000, got 1001"):
            create_sample_data(num_genres=2, tracks_per_genre=1001)
            
        with pytest.raises(ValueError, match="tracks_per_genre cannot exceed 1000, got 5000"):
...
class TestCreateSampleDataHappyPath:
    ...
    def test_various_genre_counts(self):
        ...
        for num_genres in [1, 2, 3, 4, 5]:
            ...
            assert unique_main_genres >= min(num_genres, 4)  # Max 4 main genres in current impl
...
class TestCreateSampleDataSchemaValidation:
    ...
    def test_required_columns_present(self):
        ...
        required_columns = [
            'track_id', 'track_name', 'artist_name', 'genre_tags', 
            'mood_tags', 'duration', 'energy', 'valence', 'tempo', 'genre_hierarchy'
        ]
    ...
    def test_list_columns_structure(self):
        ...
        for genre_tags in df['genre_tags']:
            ...
            assert all(isinstance(tag, str) for tag in genre_tags), "All genre tags must be strings"
        ...
        for mood_tags in df['mood_tags']:
            ...
            assert all(isinstance(tag, str) for tag in mood_tags), "All mood tags must be strings"
        ...
        for hierarchy in df['genre_hierarchy']:
            ...
            assert all(isinstance(genre, str) for genre in hierarchy), "All hierarchy items must be strings"