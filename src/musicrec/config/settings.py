"""Configuration management for the music recommender system.

This module handles configuration defaults and environment variable overrides
...
ENV_MAPPINGS = {
    ...
    "MUSICREC_MAX_RETRIES": "retry.max_attempts",
...
def _set_nested_value(config: Dict[str, Any], key_path: str, value: Any) -> None:
    ...
    if isinstance(value, str):
        if key_path.endswith('_attempts') or key_path.endswith('_port') or key_path.endswith('_limit'):
            try:
                value = int(value)
            except ValueError:
...
def load_config(config_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    if base_path:
        ...
        for key in ["spotify_path", "genre_path", "mood_path", "metadata_path", "processed_data_path"]:
